<?php $page_title="Manage News";
    ...
    if(!isset($_GET['keyword'])){
        ...
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE `news_title` LIKE '%$keyword%'";
        $targetpage = "manage_news.php?keyword=".$_GET['keyword'];
    }
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_news ORDER BY tbl_news.`id` DESC LIMIT $start, $limit"; 
    } else {
        $sql_query="SELECT * FROM tbl_news WHERE `news_title` LIKE '%$keyword%' ORDER BY tbl_news.`id` DESC LIMIT $start, $limit"; 
    }
    ...
    <div class="nsofts-container">
        ...
        <div class="card h-100">
            <div class="card-header d-md-inline-flex align-items-center justify-content-between py-3 px-4">
                ...
                <span class="ps-2 lh-1">Total News (<?=$total_pages ?>)</span>
                <div class="d-flex mt-2 mt-md-0">
                    ...
                    <a href="create_news.php?add=yes" class="btn btn-primary d-inline-flex align-items-center justify-content-center">
                        <i class="ri-add-line"></i>
                        <span class="ps-1 text-nowrap d-none d-sm-block">Create News</span>
                    </a>
                </div>
            </div>
                        ...
                        <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                            <div class="col-lg-3 col-sm-6">
                                <div class="nsofts-card-light p-3">
                                    <h5 class="mb-2"><?php echo $row['news_title'];?></h5>
                                    <div class="d-flex">
                                        <a href="<?php echo $row['news_url'];?>" class="btn btn-outline-success rounded-pill me-2 btn-icon" target="_blank" >
                                            <i class="ri-history-line"></i>
                                        </a>
                                        <a href="create_news.php?news_id=<?php echo $row['id'];?>" class="btn btn-outline-primary rounded-pill me-2 btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
