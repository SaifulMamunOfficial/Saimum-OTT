<?php $page_title="Manage Suggestion";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_suggest ORDER BY tbl_suggest.`id` DESC"; 
    } else {
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_suggest WHERE (`suggest_title` LIKE '%$keyword%') ORDER BY tbl_suggest.`id` DESC"; 
    }
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Title</th>
                                    <th class="display-desktop">Message</th>
                                    <th>User</th>
                            ...
                            <tbody id="load-more-container">
                                <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                                    <tr class="card-item">
                                        <td class="display-desktop"><img src="images/<?php echo $row['suggest_image']?>" class="image-card__cover" onerror="this.src='assets/images/300x300.jpg';"  loading="lazy" alt=""></td>
                                        <td class="text-truncate"><?php echo $row['suggest_title'];?></td>
                                        <td class="display-desktop"><?php echo $row['suggest_message'];?></td>
                                        <td class="text-truncate"><?php echo user_info($row['user_id'],"user_name");?></td>
                                        <td class="text-center">
                                            <a href="view_suggestion.php?suggestion_id=<?php echo $row['id']; ?>" class="btn btn-primary btn-icon" style="padding: 10px 10px !important;  margin-right: 10px !important;" data-bs-toggle="tooltip" data-bs-placement="top" title="View suggestion">
