<?php $page_title="Manage Banner";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_banner ORDER BY tbl_banner.`bid` DESC"; 
    } else {
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_banner WHERE (`banner_title` LIKE '%$keyword%') ORDER BY tbl_banner.`bid` DESC"; 
    }
                ...
                <?php if(mysqli_num_rows($result) > 0){ ?>
                    <div class="row g-4" id="load-more-container">
                        <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                            <div class="col-lg-6 col-sm-6 col-xs-12" class="card-item">
                                <div class="nsofts-image-card">
                                    <div class="nsofts-image-card__cover" style="width: 100%; height: 250px; min-width: 100%; min-height: 100%; max-width: 100%;">
                                        ...
                                        <img src="images/<?=$row['banner_image']?>" onerror="this.src='assets/images/600x300.jpg';"  loading="lazy" alt="">
                                    </div>
