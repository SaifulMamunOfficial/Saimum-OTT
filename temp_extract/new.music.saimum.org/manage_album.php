<?php $page_title="Manage Album";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_album ORDER BY tbl_album.`aid` DESC"; 
    } else {
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_album WHERE `album_name` LIKE '%$keyword%' ORDER BY tbl_album.`aid` DESC"; 
    }
        ...
        <div class="card h-100">
            <div class="card-top d-md-inline-flex align-items-center justify-content-between py-3 px-4">
                ...
                <div class="d-flex mt-2 mt-md-0">
                    ...
                    <a href="create_album.php?add=yes" class="btn btn-primary d-inline-flex align-items-center justify-content-center">
                        <i class="ri-add-line"></i>
                        <span class="ps-1 text-nowrap d-none d-sm-block">Create Album</span>
                    </a>
                </div>
            </div>
                                ...
                                <div class="nsofts-image-card">
                                    <div class="nsofts-image-card__cover mb-0">
                                        <div class="nsofts-switch d-flex align-items-center enable_disable" data-bs-toggle="tooltip" data-bs-placement="top" title="Enable / Disable">
                                            <input type="checkbox" id="enable_disable_check_<?= $i ?>" data-id="<?= $row['aid'] ?>" data-table="<?=$tableName ?>" data-column="status" class="cbx hidden btn_enable_disable" <?php if ($row['status'] == 1) { echo 'checked'; } ?>>
                                            <label for="enable_disable_check_<?= $i ?>" class="nsofts-switch__label"></label>
                                        </div>
                                        <img src="images/<?=$row['album_image']?>" onerror="this.src='assets/images/300x300.jpg';"  loading="lazy"  alt="">
                                    </div>
                                    <div class="nsofts-image-card__content nsofts-bottom">
                                        <div class="position-relative">
                                            <div class="d-flex align-items-center justify-content-between nsofts-image-card__content__text">
                                                <span class="d-block text-truncate fs-6 fw-semibold pe-2"><?php echo $row['album_name'];?></span>
                                                <div class="nsofts-image-card__option d-flex">
                                                    <a href="create_album.php?album_id=<?php echo $row['aid'];?>" class="btn border-0" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
                                                        <i class="ri-pencil-fill"></i>
                                                    </a>
                                                    <a href="javascript:void(0)" class="btn border-0 text-danger btn_delete" data-id="<?php echo $row['aid'];?>" data-table="<?=$tableName ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"><i class="ri-delete-bin-fill"></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
