...
public class FragmentSearchSong extends Fragment {
    ...
    private static final String TAG = "FragmentSearchSong";
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(((MainActivity) requireActivity()).getSupportActionBar()).setTitle(getString(R.string.search_songs));
    ...
    private void loadSongs() {
        ...
        }, helper.getAPIRequest(Method.METHOD_SEARCH_AUDIO, page, "", "",
                Callback.getSearchItem(), "",new SPHelper(getActivity()).getUserId(),
                "", "", "","","","","search", null));
        loadSong.execute();
    }
    ...
    private void loadSongEnd(ArrayList<ItemSong> arrayListSong) {
        if (arrayListSong.isEmpty()) {
            ...
            if (Boolean.TRUE.equals(isScroll) && Callback.getAddedFrom().equals(TAG)) {
                Callback.getArrayListPlay().clear();
                Callback.setArrayListPlay(arrayListSong);
                try {
    ...
    private void loadNativeAds() {
        if (Callback.getIsNativeAdPost() && Callback.getIsAdsStatus() && arrayList.size() >= 10) {
            switch (Callback.getAdNetwork()) {
                case Callback.AD_TYPE_ADMOB -> {
                    AdLoader.Builder builder = new AdLoader.Builder(requireActivity(), Callback.getAdmobNativeAdID());
                ...
                case Callback.AD_TYPE_STARTAPP -> {
                    StartAppNativeAd nativeAd = new StartAppNativeAd(requireActivity());
                ...
                case Callback.AD_TYPE_APPLOVIN, Callback.AD_TYPE_IRONSOURCE ->
