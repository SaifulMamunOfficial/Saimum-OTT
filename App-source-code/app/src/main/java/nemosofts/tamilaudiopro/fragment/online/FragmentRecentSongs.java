...
public class FragmentRecentSongs extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        recentSongIDs = dbHelper.getRecentIDs("200");
        if (spHelper.isLogged()){
            ...
            errorMsg = getString(R.string.login);
            setEmpty();
        }
        ...
        rv.addOnScrollListener(new EndlessRecyclerViewScrollListener(manager) {
            ...
            public void onLoadMore(int p, int totalItemsCount) {
                if (getActivity() != null && (!recentSongIDs.isEmpty() && (Boolean.FALSE.equals(isOver) && (Boolean.FALSE.equals(isLoading))))) {
                    isLoading = true;
    ...
    private void loadRecentSongs() {
        ...
        LoadAudio loadAudio = new LoadAudio(new AudioListener() {
            @Override
            public void onStart() {
            ...
            public void onEnd(String success, String verifyStatus,
                              String message, ArrayList<ItemSong> arrayListLatest) {
                if (getActivity() == null) {
                    return;
                }
                if (success.equals("1")) {
                    if (!verifyStatus.equals("-1") && !verifyStatus.equals("-2")) {
                        loadAudioEnd(arrayListLatest);
        ...
        }, helper.getAPIRequest(Method.METHOD_SONG_BY_RECENT, page, recentSongIDs,
                "", "", "", spHelper.getUserId(), "",
                "", "", "", "", "", "", null));
        loadAudio.execute();
    }

    private void loadAudioEnd(ArrayList<ItemSong> arrayListLatest) {
    ...
    public void setEmpty() {
        if (!arrayList.isEmpty()) {
            ...
            if (errorMsg.equals(getString(R.string.error_no_songs_found))) {
                ...
                btnText.setText(getString(R.string.retry));
            } else if (errorMsg.equals(getString(R.string.login))) {
                btnText.setText(getString(R.string.login));
            }
