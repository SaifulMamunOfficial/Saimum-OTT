...
import nemosofts.tamilaudiopro.activity.AlbumsByCatActivity;
...
import nemosofts.tamilaudiopro.executor.LoadArtist;
...
public class FragmentArtist extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        helper = new Helper(getActivity(), (position, type) -> openAlbumsByCatActivity(position));
        ...
        manager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                return (adapterArtist.getItemViewType(position) == -2 || adapterArtist.isHeader(position)) ? manager.getSpanCount() : 1;
            }
        });
        ...
        if(Boolean.FALSE.equals(isFromHome)) {
            rv.addOnScrollListener(new EndlessRecyclerViewScrollListener(manager) {
                ...
                public void onLoadMore(int p, int totalItemsCount) {
                    if (getActivity() != null && (!isOver && (!isLoading))) {
                        isLoading = true;
    ...
    private void openAlbumsByCatActivity(int position) {
        if(adapterArtist.getItem(position) != null) {
            Intent intent = new Intent(getActivity(), AlbumsByCatActivity.class);
    ...
    private void loadArtists() {
        ...
        String helperName = Boolean.TRUE.equals(isFromHome) ? Method.METHOD_HOME_DETAILS : Method.METHOD_ARTIST;
        LoadArtist loadArtist = new LoadArtist(new ArtistListener() {
            ...
            public void onEnd(String success,
                              String verifyStatus,
                              String message,
                              ArrayList<ItemArtist> arrayListArtist) {
                if (getActivity() == null) {
                    return;
                }
                if (success.equals("1")) {
                    loadArtistEnd(verifyStatus, message, arrayListArtist);
                } else {
                    isOver = true;
                    try {
    ...
    private void loadArtistEnd(String verifyStatus,
                               String message, ArrayList<ItemArtist> arrayListArtist) {
        if (verifyStatus.equals("-1")) {
            DialogUtil.verifyDialog(requireActivity(), getString(R.string.error_unauthorized_access), message, () -> {
            });
            return;
        }
        if (arrayListArtist.isEmpty()) {
            isOver = true;
        ...
        if (helper.canLoadNativeAds(requireContext(),Callback.PAGE_NATIVE_CAT)
            ...
            adLoader = builder.forNativeAd(nativeAd -> {
                try {
                    ...
                    if (!adLoader.isLoading() && adapterArtist != null) {
    ...
    public void setEmpty() {
        if (!arrayList.isEmpty()) {
            ...
            if (errorMsg.equals(getString(R.string.error_no_artist_found))) {
            ...
            myView.findViewById(R.id.ll_empty_try).setOnClickListener(v -> loadArtists());
