package nemosofts.tamilaudiopro.utils.encrypt;
    ...
    public void encrypt(String fileName, BufferedSource bufferedSource, final ItemSong itemSong) {
        try {
            File fileEncrypt = new File(getEditedFileName(
                    new File(fileName.concat(".mp3")), "")
            );

            final String fileSavedName = fileEncrypt.getName().replace(".mp3", "");
            ...
            String imageName = downloadAndSaveThumbnail(itemSong.getImageBig(), fileSavedName);
